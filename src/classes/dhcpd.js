...
class dhcpd {
  ...
  constructor(opts) {
    ...
    if (opts.subnet) {
      ...
      if (block) {
        ...
        throw new Error(
          "Unable to read subnet details from '" + opts.subnet + "'",
        );
      }
    }
  ...
  pre_init(pkt) {
    ...
    if (
      state == undefined ||
      state["0"] < DHCP_MINTYPE ||
      state["0"] > DHCP_MAXTYPE
    )
      return;
  ...
  discover(pkt) {
    ...
    if (
      pkt.options["60"] !== undefined &&
      pkt.options["60"].indexOf("PXEClient") === 0
    ) {
      ...
      pkt.options["60"] !== undefined &&
      pkt.options["60"].indexOf("AAPLBSDPC/i386") === 0
    ) {
      console.log("Mac OS X discovery, ignoring");
      return;
    } else {
      console.log(
        "Ignoring discovery as it does not seam to be a boot request",
      );
      return;
    }
  }
  ...
  request(pkt) {
    var self = this;
    console.log(
      "Ignoring received REQUEST from " +
        pkt.chaddr +
        " as we are only a proxy",
    );
    return;
  }
  ...
  inform(pkt) {
    ...
    if (
      ...
      offer.options["255"] = null;
      if (
        pkt.options["43"] !== undefined &&
        pkt.options["43"].indexOf("01:01:01") === 0
      ) {
        // Mac OS X client: BSDP list
        offer.options["43"] =
          "01:01:01:04:02:80:00:07:04:81:00:05:2a:08:04:81:00:05:2a:09:0d:81:00:05:2a:08:69:50:58:45:2d:46:4f:47";
      } else if (
        pkt.options["43"] !== undefined &&
        pkt.options["43"].indexOf("01:01:02") === 0
      ) {
        // Mac OS X client: BSDP select
        offer.options["43"] =
          "01:01:02:08:04:81:00:05:2a:82:0a:4e:65:74:42:6f:6f:74:30:30:31";
